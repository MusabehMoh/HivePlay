  ...
  const audioElement = streamUrl && (
    <audio
      ...
      onLoadedMetadata={() => {
        ...
        // Apply saved volume when audio element is loaded
        // This is critical to prevent volume jumps between songs
        if (audioRef.current) {
          audioRef.current.volume = volumeRef.current / 100;
        }
      }}
            ...
            <div className="flex justify-end items-center gap-2">
              <button
                ...
                className={`p-2 rounded-full transition-colors ${
                  isRepeatEnabled ? 'text-white bg-spotify-dark-highlight' : 'text-gray-400 hover:text-white'
                }`}
                disabled={!isReady}
              >
